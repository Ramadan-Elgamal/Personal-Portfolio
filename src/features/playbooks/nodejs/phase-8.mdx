---
title: "🛡️ Phase 8: Production Security Hardening (Helmet, CORS, & Rate Limiting)"
description: "Secure the application's transport layer against real-world attack vectors. We implement three automated defensive perimeters:"
language: "nodejs"
category: "playbook"
tags:
  - nodejs
...
- **Action:** Run these commands to install our security packages. _(Note: `helmet` and `express-rate-limit` include native TypeScript definitions out of the box)._
  ...
  return {
    origin: (origin, callback) => {
      ...
      if (!origin || allowedOrigins.includes(origin)) {
        ...
        callback(
          new Error(
            "🚫 CORS Error: This origin is not authorized to access the API."
          )
        )
      }
    },
...
export const authLimiter = rateLimit({
  ...
  message: {
    status: "error",
    message:
      "🚨 Too many authentication attempts. Your IP has been temporarily blocked for 1 minute.",
  },
})
```
