---
title: "🔄 Phase 15 (Optional): Real-Time Communication (Socket.io)"
description: "Establish secure, event-driven, real-time communication between the server and connected clients. We implement **Socket.io** with a strict architectural perimeter."
language: "nodejs"
category: "playbook"
tags:
  - nodejs
...
_(Note: Socket.io v4+ includes comprehensive native TypeScript definitions out of the box)._
...
export interface ServerToClientEvents {
  ...
  "link:processing:complete": (payload: {
    linkId: string
    title: string
    url: string
  }) => void
...
This module initializes the Socket.io server, applies our strict CORS parameters, binds the Redis adapter, and enforces authentication interceptors _before_ granting connection access.
...
export const initializeSocketServer = async (
  ...
  try {
    ...
    logger.error(
      { err: error },
      "❌ Failed to bind Socket.io Redis Adapter. Running in standalone mode."
    )
  }
  ...
  io.use((socket, next) => {
    ...
    if (!token) {
      return next(
        new Error(
          "Authentication error: Valid Bearer token required for connection upgrade."
        )
      )
    }
  ...
  io.on("connection", (socket) => {
    ...
    logger.info(
      `🔗 Socket connected [ID: ${socket.id}] - Subscribed to room [${userRoom}]`
    )
...
export const linkWorker = new Worker(
  ...
  async (job: Job<ILinkMetadataJob>) => {
    ...
    try {
      ...
      logger.info(
        `✅ [Worker] Pushed complete notification for Link ID: ${linkId}`
      )
    } catch (error: any) {
